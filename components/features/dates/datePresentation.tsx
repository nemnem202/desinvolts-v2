...
export default function DatePresentation({
	...
	const currentSortedIndex = sortedDates.findIndex(
		(d) => d.id === currentDate?.id,
	);
	...
	if (!currentDate)
		return (
			<AddDateDialog onDate={(newDate) => setDates([...dates, newDate])} />
		);
		...
		<>
			<div className="flex flex-col w-full items-center gap-4 p-4 relative">
				...
				<div className="flex w-full justify-between items-center">
					...
					<NavButton
						disabled={currentSortedIndex >= sortedDates.length - 1}
						onClick={goNext}
					>
				...
				<div
					className={`w-full flex flex-col items-center ${isPast && "opacity-40"}`}
				>
					<EditableText
						as="h2"
						...
						setContent={(newText) => {
							setDates(
								dates.map((d) =>
									d.id === currentDate.id ? { ...d, title: newText } : d,
								),
							);
						}}
						className="title"
					...
					<div className="flex flex-col items-center md:items-start md:flex-row md:justify-center w-full max-w-[50rem] gap-4">
						<div className="w-[15rem] md:w-[20rem] aspect-2/3 overflow-hidden rounded-md shrink-0">
							<Image
								...
								onChange={(newImage) => {
									setDates(
										dates.map((d) =>
											d.id === currentDate.id ? { ...d, image: newImage } : d,
										),
									);
								}}
							/>
						...
						<div
							...
							onClick={() =>
								(size === "sm" || size === "md") && setCalendarOpen(true)
							}
						>
										...
										{isAdminDisplay && (
											<div className="w-full flex justify-center mt-2">
												<ButtonMinus
													onClick={() =>
														setDates(
															dates.filter((d) => d.id !== currentDate.id),
														)
													}
												/>
											</div>
								...
								<div className="flex-1">
									<DayPicker
										...
										onValueChange={(newDate) => {
											setDates(
												dates.map((d) =>
													d.id === currentDate.id ? { ...d, date: newDate } : d,
												),
											);
										}}
									/>
								...
								<div className="flex-1">
									...
									<EditableText
										...
										setContent={(newText) => {
											setDates(
												dates.map((d) =>
													d.id === currentDate.id ? { ...d, city: newText } : d,
												),
											);
										}}
									/>
